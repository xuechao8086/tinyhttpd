  ...
  [cs]: http://www.owlfish.com/
    ...
    class WSGIHandler(SimpleHTTPServer.SimpleHTTPRequestHandler):
        def run_wsgi_app(self):
            ...
            for http_header, http_value in slef.headers.items():
                env['HTTP%s'% http_header.replace('-', '_').upper()] = \
                        http_value

            self.wsgi_send_headers = 0
            self.wsgi_headers = []
        ...
        def wsgi_start_response(self, response_status, response_headers,
                exc_info=None):
            if(self.wsgi_send_headers):
                raise Exception \
                        ("Headers already send and start_response call again!")
