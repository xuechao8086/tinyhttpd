...
/**********************************************************************/
void* accept_request(void *client_args) {
    char buf[1024];
    int numchars;
    char method[255];
    char url[255];
    ...
    sprintf(path, "/home/charlie/tinyhttpd/%s", url);
...
void bad_request(int client) {
    ...
    send(client, buf, sizeof(buf), 0);
    sprintf(buf, "such as a POST with a Content-Length.\r\n");
}
...
void not_found(int client) {
    ...
    send(client, buf, strlen(buf), 0);
    sprintf(buf, "<BODY><P>The server count not fulfill\r\n");
    send(client, buf, strlen(buf), 0);
